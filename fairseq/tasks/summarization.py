...
class SummarizationTask(FairseqTask):
    ...
    def reduce_metrics(self, logging_outputs, criterion):
        ...
        try:
            ctc_loss = sum(log.get("ctc_loss") for log in logging_outputs)
            reconstruction_loss = sum(log.get("reconstruction_loss") for log in logging_outputs)
            gan_loss = sum(log.get("gan_loss") for log in logging_outputs)
            metrics.log_scalar("CTC loss ", ctc_loss, priority=1, round=2)
            metrics.log_speed("Reconstruction loss", reconstruction_loss, priority=2, round=2)
            metrics.log_speed("GAN loss", gan_loss, priority=3, round=2)
        except:
            pass
