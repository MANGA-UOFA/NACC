...
class ATScopeSearchCharLengthControlDecoder(CTCDecoderBase):
    """
    Length Control decoder for AT models (e.g., Transformer)
    """
    ...
    def __init__(self, dictionary, decoder_parameters):
        ...
        self.device = None
        self.dtype = None
        self.id_to_index_tensor = None
        self.prev_id_to_index_tensor = None
        ...
        self.blank_id = dictionary.pad()  # Change it to the last token in the dictionary, which is very impossible
    ...
    def top_k_filtering(self, column, lprob):
        ...
        top_k_id_tensor[0] = self.blank_id
        top_k_lprob_tensor[0] = -20  # use some low value to disencourage selection of this word
        ...
        top_k_id_tensor[1:] = naive_top_k_id[naive_top_k_id != self.blank_id][:self.k - 1]
        top_k_lprob_tensor[1:] = naive_top_k_lprob[naive_top_k_id != self.blank_id][:self.k - 1]
        ...
        self.id_to_index_tensor[list(id_to_index_dict.keys())] \
            = torch.tensor(list(id_to_index_dict.values())).to(self.device)

        # We don't consider any tokens as repeated tokens, dropping the CTC merging rules
        ...
        self.prev_id_to_index_tensor[list(id_to_index_dict.keys())] \
            = torch.tensor(list(id_to_index_dict.values())).to(self.device)
    ...
    def scope_search_row_inference(self, column, current_max_row_index, reshaping_index, filtered_lprob_matrix,
        ...
        row_history_reshaping_index = tuple([...] + [0, None]*(num_dummy_dim-1) + [[*range(self.ctc_sequence_length)]])
        length_history_reshaping_index = tuple([...] + [0, None]*(num_dummy_dim-1))
        row_history = self.transition_tracker_table[:self.prev_max_row_index + 1][row_history_reshaping_index]
        ...
        # Record
        correct_row_shape = self.transition_tracker_table[:current_max_row_index + 1][row_history_reshaping_index].shape
    ...
    def scope_search_column_inference(self, column):
        ...
        lprob = self.prob_sequence[column].log()
        filtered_lprob, repeated_or_special_element_index_list, top_k_id_tensor = self.top_k_filtering(column, lprob)
    ...
    def margin_over_prob_table(self):
        ...
        self.scope_length_table[0:self.prev_max_row_index + 1] \
            = self.scope_length_table[row_axis, most_probable_word_index].unsqueeze(-1).repeat(repeat_index)
    ...
    def find_length_mask(self, column, current_max_row_index, current_k_words_length, num_dummy_dim=None,
        ...
        if column == 0:
            ...
            # Return the length mask
            length_mask = (total_length > length_lower_bound) & (total_length <= length_upper_bound)
            return length_mask
    ...
    def ctc_scope_search_length_control_initialization(self, logits):
        ...
        self.transition_tracker_table = \
            torch.zeros(transition_tracker_dimensions, dtype=torch.long, device=self.device) - 1
        ...
        self.id_to_index_tensor = torch.zeros(len(self.word_length_tensor), dtype=torch.long, device=self.device)
        self.prev_id_to_index_tensor = torch.zeros(len(self.word_length_tensor), dtype=torch.long, device=self.device)
    ...
    def at_scope_search_char_length_control(self, logits, source_length):
        """
    ...
    def decode(self, finalized, source_length):
        # TODO: change the architecture such that source_length is the number of chars in the source
        finalized_prob_sequence = [finalized[i][0]["prob_sequence"] for i in range(0, len(finalized))]
        finalized_summary_list = [finalized[i][0]["tokens"] for i in range(0, len(finalized))]
        self.dtype = finalized_prob_sequence[0].dtype
        self.device = finalized_prob_sequence[0].device
        decoded_summary_list = []
        for i in range(0, len(finalized)):
            decoded_summary = finalized_summary_list[i]
            decoded_summary_mask = (decoded_summary != self.dictionary.eos_index) & \
                                   (decoded_summary != self.dictionary.bos_index) & \
                                   (decoded_summary != self.dictionary.pad_index)
            decoded_summary = decoded_summary[decoded_summary_mask]
            decoded_summary_length = self.word_length_tensor[decoded_summary].sum() + len(decoded_summary) - 1
            if decoded_summary_length <= self.desired_length:
                pass
            else:
                current_logits = finalized_prob_sequence[i]
                decoded_summary = self.at_scope_search_char_length_control(current_logits, source_length[i])
